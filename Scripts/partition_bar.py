...
def make_bar(pheno, df1, df2=pd.DataFrame({'A' : []})):
    ...
    if df2.empty:
        ...
        plt.errorbar(x-0.2, df1['Enrichment'], yerr = (df1['Enrichment_std_error']), linestyle="none", color="darkslategray", capsize=3, label = "2 SE")
        plt.xticks(fontsize="large")
        plt.title("Partitioned Heritability for {0} - {1}".format(pheno, result.capitalize()), fontsize="large")
