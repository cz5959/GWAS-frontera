...
for(i in p.threshold){
    ...
    pgs_f <- pgs_f[pgs_f$IID %in% f_ids$IID,]
    pgs_m <- read.table(paste0("male_",type,"_",pheno,".",i,".profile"), header=T)
    pgs_m <- pgs_m[pgs_m$IID %in% m_ids$IID,]
...
print(paste0("Best result for combined, ", type))
...
save(combined_pgs.summary, file= paste0(pheno,"_",type,"_pgs_results.RData"))