...
effect_labels <-  c('female','female x3', 'female x2', 'female x1.5','equal','male x1.5','male x2','male x3','male')
big <- ggplot(df_ave, aes(x= magnitude, y= correlation, fill= Mean)) +
  ...
  labs(title="Weights of Hypothesis Matrices") +
  ...
  scale_fill_gradient(low="gray85",high="#1d47a1")
...
small <- ggplot(df_null, aes(x= 0, y= 0, fill= Mean)) +
  ...
  scale_fill_gradient(low="#1d47a1",high="#1d47a1")
...
ggplot(df_small, aes(x=magnitude, y= correlation, fill= value)) +
  geom_tile(color= "white", lwd= 1.5, linetype= 1) +
  geom_text(aes(label=sprintf("%.0f%%", (value*100))), color= "white", size= 3.2) +
  labs(title="Weights of Hypothesis Matrices") +
  xlab("Magnitude") + ylab("Correlation") +
  theme_pubclean() +
