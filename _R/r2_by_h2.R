...
p1 <- ggplot(df, aes(x=relative_h2, y=Phenotype, col=(Sex))) +
  ...
  geom_point(aes(x=relative_h2-0.4, shape=as.factor(star)), color="red", size=4) +
  ...
  # legend
  geom_rect(inherit.aes = FALSE, xmin=1.75,xmax=2.1, ymin=20,ymax=25, fill="white") +
  annotate("text", x=1.8, y=24, label="Sex", hjust=0, fontface=2) +
  annotate("text", x=1.8, y=23, label="male", hjust=0, color="#207335") +
  annotate("text", x=1.8, y=22, label="both sex", hjust=0, color="#1d47a1") +
...
p <- grid.arrange(p2, p1, ncol = 2, layout_matrix=lay, 
                  top=textGrob("Distribution of Heritability in Relation to Genetic Correlation", gp=gpar(fontsize=16)))
p  
