...
write.table(pheno_var, file="pheno_var2.txt",sep="\t",quote=FALSE)
#pheno_var <- read.csv("pheno_var.txt", sep="\t")
setwd("~/Research/GWAS-frontera/Phenotypes")
pheno_var <- read.csv("pheno_var2.txt", sep="\t")
...
g2 <- ggplot(empty_df) + geom_point() +
  geom_hline(yintercept = 24.8) +
  scale_x_continuous(breaks=c(3,3.4,3.8), limits=c(2.9,4), expand=c(0,0)) +
  scale_y_continuous(breaks=c(23.5,24,24.5), limits=c(23.4,24.8), expand=c(0,0)) +
  theme_classic() +
...
g3 <- ggplot(data=df3, aes(x= geno_var_ratio, y= env_var_ratio)) +
  geom_hline(yintercept = 24.8) + geom_vline(xintercept = 92) +
  geom_point(size=2, alpha=0.5) + 
  ...
  scale_y_continuous(breaks=c(23.5,24,24.5), limits=c(23.4,24.8), expand=c(0,0)) +
  scale_x_continuous(breaks=c(70,80,90), limits=c(63,92), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text=element_blank(), axis.title=element_blank(), axis.ticks=element_blank(),
        axis.line=element_blank()) +
...
g3 <- ggplot(data=df3, aes(x= geno_var_ratio, y= env_var_ratio)) +
  ...
  scale_y_continuous(breaks=seq(23,25,1), limits=c(22.47,25.7), expand=c(0,0)) +
  scale_x_continuous(breaks=seq(60,100,20), limits=c(50,108), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text=element_blank(), axis.title=element_blank(), axis.ticks=element_blank(),
        axis.line=element_blank()) +
  geom_text(aes(label=pheno_name), size=anno_size,x=75,y=24) +
...
g1 <- ggplot(empty_df) + geom_point() + 
  ...
  scale_y_continuous(breaks=seq(22,26,2), limits=c(20,28), expand=c(0,0)) + 
  scale_x_continuous(breaks=seq(0.5, 2, 0.5), limits=c(0,2.4), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text.x=element_blank(), axis.text.y = element_text(size=10), axis.title=element_blank(), 
        axis.ticks.x=element_blank(), axis.line.x = element_blank()) +
  annotate("text", x = 0.5, y = 24.5, label= "Consistent with 1:1 Relationship", hjust = 0, color = pcolor) +
...
g2 <- ggplot(empty_df) + geom_point() +
  geom_hline(yintercept = 28) +
  scale_x_continuous(breaks=seq(2.5,4.5,0.5), limits=c(2.4,4.6), expand=c(0,0)) +
  scale_y_continuous(breaks=seq(22,26,2), limits=c(20,28), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text=element_blank(), axis.title=element_blank(), axis.ticks=element_blank(),
        axis.line = element_blank())
...
g3 <- ggplot(data=df3, aes(x= geno_var_ratio, y= env_var_ratio)) +
  geom_hline(yintercept = 28) + geom_vline(xintercept = 125) +
  geom_point(size=2, alpha=0.5) + 
  geom_errorbar(aes(y=env_var_ratio, ymin=env_var_ratio-3*env_var_ratio_se, ymax=env_var_ratio+3*env_var_ratio_se), width=0) +
  geom_errorbarh(aes(y=env_var_ratio, xmin=geno_var_ratio-3*geno_var_ratio_se, xmax=geno_var_ratio+3*geno_var_ratio_se), height=0) +
  scale_y_continuous(breaks=seq(22,26,2), limits=c(20,28), expand=c(0,0)) +
  scale_x_continuous(breaks=seq(40,120,40), limits=c(35,125), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text=element_blank(), axis.title=element_blank(), axis.ticks=element_blank(),
        axis.line=element_blank()) +
  geom_text(aes(label=pheno_name), size=anno_size,x=75,y=24) +
...
g4 <- ggplot(data=df1, aes(x= geno_var_ratio, y= env_var_ratio, color=on)) +
  geom_point(size=2, alpha=0.5) + 
  geom_abline(slope = 1, intercept = 0, color=pcolor, size=0.8) +
  geom_errorbar(aes(y=env_var_ratio, ymin=env_var_ratio-3*env_var_ratio_se, ymax=env_var_ratio+3*env_var_ratio_se), 
                width=0) +
  geom_errorbarh(aes(y=env_var_ratio, xmin=geno_var_ratio-3*geno_var_ratio_se, xmax=geno_var_ratio+3*geno_var_ratio_se),
                 height=0) +
  scale_y_continuous(breaks=c(1,2,3), limits=c(0,3.5), expand=c(0,0)) +
  scale_x_continuous(breaks=seq(0.5, 2, 0.5), limits=c(0,2.4), expand=c(0,0)) +
  theme_classic() +
...
g5 <- ggplot(data=df2, aes(x= geno_var_ratio, y= env_var_ratio)) +
  geom_point(size=2, alpha=0.5, color=pcolor) + 
  geom_abline(slope = 1, intercept = 0, color=pcolor, size= 0.8) +
  geom_errorbar(aes(y=env_var_ratio, ymin=env_var_ratio-3*env_var_ratio_se, 
                    ymax=env_var_ratio+3*env_var_ratio_se), width=0, color=pcolor) +
  geom_errorbarh(aes(y=env_var_ratio, xmin=geno_var_ratio-3*geno_var_ratio_se, xmax=geno_var_ratio+3*geno_var_ratio_se), 
                 height=0, color=pcolor) +
  scale_x_continuous(breaks=seq(2.5,4.5,0.5), limits=c(2.4,4.6), expand=c(0,0)) +
  scale_y_continuous(breaks=c(1,2,3), limits=c(0,3.6), expand=c(0,0)) +
  theme_classic() +
  theme(axis.text.y=element_blank(), axis.text.x = element_text(size=10), axis.title=element_blank(), 
        axis.ticks.y=element_blank(), axis.line.y = element_blank(), legend.position = "none") +
  geom_text_repel(aes(label=pheno_name, fontface = on), size=anno_size, max.overlaps=Inf, 
                  segment.alpha=0, segment.linetype=2, color=pcolor) +
  annotate("text", x=3.3, y=2.3, label = "1:1 Relationship", color= pcolor)
...
########################################################## p value
x <- df1$geno_var_ratio; x.se <- df1$geno_var_ratio_se
y <- df1$env_var_ratio; y.se <- df1$env_var_ratio_se
se_ratio <- (x/y)*sqrt((x.se^2/x^2)+(y.se^2/y^2))
z_ratio <- ((x/y)-1) / se_ratio  
df1$p <- pnorm(z_ratio)
