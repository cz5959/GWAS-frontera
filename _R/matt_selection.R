...
num <- commandArgs(trailingOnly = TRUE)[1]
p <- as.numeric(num)

cat("P-value Threshold:",num)

# Use a list of ancestries (as abbreviations) from Gnomad which were the basis
# of investigation, and store corresponding "Full" ancestry name
AncList <- data.frame(abr = c("afr","amr","asj","eas","fin","nfe","sas","oth"),
                      full = c("African","American Admixed/Latino",
                               "Ashkenazi Jewish","East Asian","Finnish",
                               "Non-Finnish European","South Asian","Other"))

# Get list of ancestry mean and median sample sizes for XX and XY individuals
AncSizes <- read.csv("./AvgSampleSizes.csv",header = TRUE)
...
FullList <- data.frame(abr = c(AncList$abr,TraitList$abr),
                       full = c(AncList$full,TraitList$full))

# Setting up universal variables used in generating all FST vs. V plots
blocks <- read.table("./Pickrell_breakpoints_EUR.bed",header = TRUE)
...
FST_Vplot <- function(a,t,u){
  ...
  set.seed(100) # Allow for reproducible results when re-running script
  tFull <- abrToFull(t)
  aFull <- abrToFull(a)
  ...
  pstats <- read.table(paste0("./",t,"/pstats.",t,".txt"),header = TRUE)
  ...
  di <- read.table(paste0("./",t,"/",a,"/",a,".table.",t,".txt"),header = TRUE)
  ...
  points <- data.frame(V = rep(NA,3406),
                       FST = rep(NA,3406),
                       varV = rep(NA,3406),
                       Samp = rep(NA,3406))
  ...
  for(i in 1:length(blocks$chr)){
    ...
    R <- d[samp,3:5]
    R$Samp <- "R"
    points[i*2-1,] <- R  # Add that locus' V, FST, and var[V] to points
  ...
  save(pointsf, file=paste0("fst_plot_",t,".RData")) ### cz
  ...
  p1 <- ggplot(pointsf) + 
    ...
    ggtitle(paste("FST vs V for",tFull,"\nin the",aFull,"Population")) +
    xlab(xax) +
    ylab(bquote("Male\u2013Female " ~F[ST])) + 
  ...
  of1 <- paste0(t,".",a,".FST_V.jpeg")
  ggsave(of1,plot = p1,dpi = 300, device = "jpeg")
  return(p1)
}
...
z <- read.csv(paste0("./results.",num,".z.csv"),header = TRUE)
...
ZscorePlot <- function(Ancs){
  ...
  resultsi$LAB <- c("","Mean Z-score for Ancestry","")
  ...
  results$COLS <- ifelse(results$ANC == a1 & results$TRAIT == t1,"col1",
                         ifelse(results$ANC == a2 & results$TRAIT == t2,"col2",
                                "b"))
  ...
  save(resultsi, results, data, file="zscore_plot.RData") ### cz
  ...
  p2 <- ggplot() + 
    ...
    geom_errorbar(data = results,
                  aes(ZMEAN,TRAIT,xmin = ZMEAN - ZSE,xmax = ZMEAN + ZSE,
                      color = COLS),
                  width = .5) +
    ...
    geom_text(data = resultsi,mapping = aes(x = -0.2,y = 34,label = c("","Zero","")),
  ...
  of2 <- paste0("zscores.",num,".jpeg")
  ggsave(of2,plot = p2,dpi = 300,device = "jpeg",width = 11)
  return(p2)
}
...
opdf <- paste0("FullFig.",num,".pdf")

# ggsave(ojpeg,plot = figfinal,path = "./Figs/Main/",device = "jpeg",
#        width = 20,height = 18)
ggsave(opdf,plot = figfinal,device = cairo_pdf,
