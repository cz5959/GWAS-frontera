...
# ids.txt
cut -f2 $meta_path/ukb45020.txt > iids.txt
paste iids.txt iids.txt > ids.txt
...
awk -F "\t" '{print $10045}' $meta_path/ukb45020.txt | paste -d "\t" ids.txt - | awk -F "\t" '{for(i=1;i<=NF;i++){if($i==""){next}}}1' - | cut -f1,2 - > in_pca_ids.txt
awk -F "\t" '{print $10044}' $meta_path/ukb45020.txt | paste -d "\t" ids.txt - | awk -F "\t" '{for(i=1;i<=NF;i++){if($i==""){next}}}1' - | cut -f1,2 - > sex_aneuploidy.txt
