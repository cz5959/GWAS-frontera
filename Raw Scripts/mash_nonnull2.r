...
t <- (mash_BETA$female - mash_BETA$male) / sqrt( mash_SE$male^2 + mash_SE$female^2)
p <- pt(abs(t), 149999, lower.tail=FALSE)
p_id <- which(p<1e-8)
...
mixture_ttest <- mash(mash_BETA, mash_SE)
write.table(mixture_ttest, file=paste0(name,"_ttest.txt"),row.names = FALSE, sep="\t")
