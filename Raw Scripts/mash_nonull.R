...
option_list = list(
  ...
  make_option(c("-f","--female"), type="integer", default="0",help="female proportion",metavar="character"),
  make_option(c("-m","--male"), type="integer", default="0",help="male proportion",metavar="character"),
  make_option(c("-s","--scale"), type="integer", default="0",help="scale multiplier",metavar="character")
)
opt_parser = OptionParser(option_list=option_list)
opt = parse_args(opt_parser)
name <- opt$name; print(name)
f <- opt$female; m <- opt$male; s <- opt$scale
setwd("/scratch/08005/cz5959/simulation")
load(paste0(name,".RData"))

signs <- which(sign(mash_BETA$female) != sign(mash_BETA$male))
mash_BETA$male[signs] <- mash_BETA$male[signs] * -1
name <- paste0(name,"_",f,"_",m,"_",s,"_same")
# % female, % neither, % male
set.seed(1)
num <- 1:nrow(mash_BETA)
f <- sample(num, length(num)*(f/100))
m <- sample(num[-f], length(num)*(m/100))
b <- num[-c(f,m)]

mash_BETA$female[f] <- mash_BETA$female[f] * s
mash_BETA$male[m] <- mash_BETA$male[m] * s
