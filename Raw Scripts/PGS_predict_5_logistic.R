...
library(ROCR, lib.loc="/work/08005/cz5959/frontera/R/x86_64-pc-linux-gnu-library/4.0/")
# argument parser
option_list = list(
...
phenotype <- read.table(pheno_file, sep="\t", head=FALSE, col.names=c("FID","IID",pheno), colClasses=c("integer","integer","factor"))
...
pgs_null <- function(sex) {
    ...
    # null model
    null.model <- glm(paste0(pheno," ~ ."), family="binomial"(link='logit'), data=pheno_covar_null[,!colnames(pheno_covar_null) %in% c("FID","IID")])
    null.pheno = predict(null.model,type="response")
    null.pred <- prediction(null.pheno, pheno_covar_null[pheno])
    null.auc <- performance(null.pred, measure = "auc")@y.values[[1]]

    print(paste0("Null AUC for ", sex, ": ", null.auc))
    return(null.auc)
}

pgs_prediction <- function(sex, type = "additive", null.auc, mode = 'specific') {  
    pgs.result <- NULL
    for(i in p.threshold){
        ...
        # logistic regression
        model <- glm(paste0(pheno," ~ ."), family="binomial", data=pheno.pgs[,!colnames(pheno.pgs)%in%c("FID","IID")])
        model.pheno = predict(model,type="response")
        model.pred <- prediction(model.pheno, pheno.pgs[pheno])
        model.auc <- performance(model.pred, measure = "auc")@y.values[[1]]
        pgs.auc <- model.auc-null.auc                  # incremental AUC
        ...
        # store results
        pgs.result <- rbind(pgs.result, data.frame(Threshold=i, AUC=model.auc, incAUC=pgs.auc, P=pgs.p, BETA=pgs.beta, SE=pgs.se, SNP=pgs.snps))        
    }
    # Best threshold based on AUC is:
    print(paste("Best result for", sex, type, mode, sep=", "))
    print(pgs.result[which.max(pgs.result$AUC),])
    # return results
