...
setwd(wd)
pgs_prediction <- function(sex) {  
    # test set
    test_ids <- read.table(paste0(pheno,"_",sex,"_testIIDs.txt"), sep="\t", col.names=c("FID","IID"))
    ...
    for(i in p.threshold){
        ...
        # store results
        pgs.result <- rbind(pgs.result, data.frame(Threshold=i, R2=model.r2, incR2=pgs.r2, P=pgs.p, \
        BETA=pgs.beta, SE=pgs.se, SNP=pgs.snps))
...
female_pgs.summary <- pgs_prediction("female")
male_pgs.summary <- pgs_prediction("male")

if (type == "additive") {
    both_sex_pgs.summary <- pgs_prediction("male")
    save(both_sex_pgs.summary, female_pgs.summary, male_pgs.summary, file= paste0(pheno,"_",type,"_pgs_results.RData"))
} else {
    save(female_pgs.summary, male_pgs.summary, file= paste0(pheno,"_",type,"_pgs_results.RData"))
}
